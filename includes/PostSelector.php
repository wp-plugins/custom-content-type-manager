...
*/
	...
	#	$r['preview_html'] = htmlentities($r['preview_html']); // in case the description has quotes in it.
		...
		// Make sure this HTML matches what's generated by PHP in the FormGenerator
		// Use this format: http://site.com/?p=123&preview=true
/*
		$preview_html = htmlentities($preview_html); // in case the description has quotes in it.
		$site_url = get_site_url(); #get_option('siteurl');
		$preview_html .= '<span class="formgenerator_label">'
			. $r['post_title']
			.' <span class="formgenerator_id_label">('.$r['post_id'].')</span> 
			<a href="'.$site_url.'?p='.$r['post_id'].'&preview=true" target="_blank">'
			.__('View').'</a></span><br />';
		$preview_html = preg_replace('/"/', "'", $preview_html); 
		$preview_html = preg_replace("/'/", "\'", $preview_html);
		$r['preview_html'] = $preview_html;
