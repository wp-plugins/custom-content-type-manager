...
class Columns {
	 ...
	 */
	public function __callStatic($post_type, $default_columns) {
...
//		print_r($raw_columns); exit;
	 ...
	 * column, so we rely on the self::$no_title_flag boolean variable (set in __call)
	 ...
	 */
	public static function populate_custom_column_data($column) {
		
		// Let users rely on their own callback function
		// See https://code.google.com/p/wordpress-custom-content-type-manager/wiki/CustomColumns
		$function_name = 'cctm_custom_column_'.self::$post_type.'_'.$column;
