...
var popUpCal = {
	...
	formatDate: function(day, month, year, hour, minute) {
		...
		if (this.dateFormat == 'yyyy-mm-dd hh:mm:ss') {
			dateString += year+'-'+pad(month,2)+'-'+pad(day, 2) +' '+pad(hour,2)+':'+pad(minute,2)+':00';
		}
		else if (this.dateFormat == 'mm/dd/yy hh:mm') {
			var year_str = String(year);
			dateString += pad(month,2)+'/'+pad(day, 2)+'/'+year_str.substr(2,2) +' '+pad(hour,2)+':'+pad(minute,2);		
		} 
		else if (this.dateFormat == 'mm/dd/yy hh:mm am') {
			var year_str = String(year);
			dateString += pad(month,2)+'/'+pad(day, 2)+'/'+year_str.substr(2,2) +' '+pad(hour,2)+':'+pad(minute,2);
			if (hour < 12) {
				dateString += ' AM';		
	...
	showCalendar: function() {
		...
		jQuery('#calendar_div').empty().append(html).show(this.speed);
		this.input[0].focus();
		this.setupActions();
	}, // end showCalendar
	...
	setupActions: function() {
		if (this.timeSeparators && this.timeSeparators.length > 1) {
			...
			jQuery("#calendar_minute").change(function() {	//change minute
				popUpCal.selecting = false;
				popUpCal.selectedMinute = this.options[this.selectedIndex].value - 0;
				popUpCal.adjustDate(); 
				popUpCal.input.val(popUpCal.formatDate(popUpCal.selectedDay, popUpCal.selectedMonth, popUpCal.selectedYear, popUpCal.selectedHour, popUpCal.selectedMinute));
			}).click(this.selectMonthYear);;
			jQuery("#calendar_ampm").change(function() {		//change am/pm
				popUpCal.selecting = false;
				if (popUpCal.timeSeparators.length == 4) {
					var ampm = jQuery('#calendar_ampm').val();
		...
		jQuery('.calendar_daysRow td a').hover( // highlight current day
             function() {
				jQuery(this).addClass('calendar_daysCellOver');
            }, function() {
				jQuery(this).removeClass('calendar_daysCellOver');
        });
        jQuery('.calendar_daysRow td a').click(function() { // select day
            popUpCal.selectedDay = jQuery(this).html();
            popUpCal.selectDate();
        });
...
jQuery.fn.datetimepicker = function(settings) {
