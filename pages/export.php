...
<div class="wrap">
	...
	<h2>Export</h2>
		...
		<form id="cctm_export_form"  method="post">
			...
			<input type="text" name="title" class="<?php print self::_get_class('title', 'text'); ?>" id="title" value="<?php print self::get_value($settings['export_info'], 'title'); ?>" /><br/>
			
			<label for="author" class="<?php print self::_get_class('author', 'text_label'); ?>" id="author_label"><?php _e('Author', CCTM_TXTDOMAIN); ?></label><br/>
			<input type="text" name="author" class="<?php print self::_get_class('author', 'text'); ?>"id="author" value="<?php print self::get_value($settings['export_info'], 'author'); ?>" /><br/>
		
			<label for="url" class="<?php print self::_get_class('url', 'text_label'); ?>" id="author_label">Your URL</label><br/>
			<input type="text" name="url" class="<?php print self::_get_class('url', 'text'); ?>" id="url" size="60" value="<?php print self::get_value($settings['export_info'], 'url'); ?>" /><br/>
			...
			<textarea name="description" class="<?php print self::_get_class('description', 'textarea'); ?>" id="description" rows="5" cols="40"><?php print self::get_value($settings['export_info'], 'description'); ?></textarea>
			<br/>
	...
	<?php include(CCTM_PATH.'/includes/footer.php'); ?>
